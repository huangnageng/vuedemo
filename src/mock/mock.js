...
export default {
  ...
  start() {
    ...
    mock.onPost("/todo/changeRecord").reply(config => {
      let { id, item } = JSON.parse(config.data);
      // id 是传进来的值唯一待办项的id
      // text 用户新增输入的数据
      Todos.some((t, index) => {
        if (t.id === id) {
          for (let i = 0, len = t.record.length; i < len; i++) {
            if (t.record[i].id === item.id) {
              Object.assign(t.record[i], item);
            }
          }
          return true;
        }
